  ...
  const handleDel = async () => {
    if (note.id) {
      ...
      try {
        ...
        if (
          (res.data.success && res.data.msg == "public_note_del") ||
          res.data.msg == "private_note_del"
        ) {
    ...
    <div className={styleMain}>
      <h1
        className={`font-poppins break-all ${
          note.note.search(" ") > 27 && "truncate"
        } ${note.note.search(" ") == -1 && "truncate"}`}
      >
      ...
      <div className="flex items-center ">
        <PencilAltIcon
          onClick={() => handleEdit()}
          className="h-5 w-5 text-black mr-4 hover:text-blue-700"
        />
        {load ? (
          ...
          <TrashIcon
            onClick={() => handleDel()}
            className="h-5 w-5 text-black mr-4 hover:text-pink-800 "
          />
        )}
        <h1 className="font-poppins font-semibold ml-2">
          {note.shared_by ? "Public" : "Private"}
        </h1>
      </div>
    </div>
  );
